...
def main():
    ...
    parser.add_argument("--input_dir", type=str, required=True, help="Directory containing test .npy arrays")
    parser.add_argument("--output_dir", type=str, required=True, help="Directory to save restored .npy arrays")
    ...
    # We dynamically load config to initialize model with correct parameters
    ...
    # Group files by shape for clean batch processing (since shape might differ)
    shape_groups = {}
    for fname in filenames:
        fpath = os.path.join(input_dir, fname)
        # Load header only to get shape quickly
        ...
        for shape, files in shape_groups.items():
            ...
            for i in range(0, len(files), batch_size):
                ...
                for fname in batch_files:
                    ...
                    batch_data.append(normalize(arr))
                ...
                for idx, fname in enumerate(batch_files):
                    ...
                    # Denormalize back to the original physical scale
    ...
    print(f"Restored arrays saved to: {output_dir}")
